...
if [ ! -f "$WEBUI_SCRIPT" ]; then
    echo -e "${RED}ERROR: webui.py not found at: $WEBUI_SCRIPT${NC}"
    exit 1
fi

echo -e "${GREEN}[1/5] Backing up current webui.py on server...${NC}"
ssh "${USER}@${SERVER}" "cp /usr/local/bin/pmranger/scripts/webui.py /usr/local/bin/pmranger/scripts/webui.py.backup-\$(date +%Y%m%d_%H%M%S) 2>/dev/null || echo 'No existing file to backup'"

echo -e "${GREEN}[2/5] Uploading fixed webui.py to server...${NC}"
if ! scp "$WEBUI_SCRIPT" "${USER}@${SERVER}:/tmp/webui.py.new"; then
    echo -e "${RED}ERROR: Failed to upload webui.py to server${NC}"
    echo -e "${YELLOW}Make sure you can SSH to the server: ssh $USER@$SERVER${NC}"
    exit 1
fi

echo -e "${GREEN}[3/5] Installing new webui.py...${NC}"
...
echo -e "${GREEN}[4/5] Restarting hotswap-webui service...${NC}"
...
echo ""
ssh "${USER}@${SERVER}" "systemctl status hotswap-webui --no-pager -l" || true
...
echo "  ssh ${USER}@${SERVER} 'journalctl -u hotswap-webui -f'"
echo ""
echo -e "${CYAN}To rollback if needed:${NC}"
echo "  ssh ${USER}@${SERVER}"
echo "  cd /usr/local/bin/pmranger/scripts"
echo "  cp webui.py.backup-YYYYMMDD_HHMMSS webui.py"
